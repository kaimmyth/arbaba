                            ...
                            <table id="datatable-responsive" class="table table-striped table-bordered dt-responsive" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                                ...
                                <tbody>
                                 ...
                                 @foreach ($toReturn as $items)
                                    ...
                                    <td>{{$items['detail_type']}}</td>
                                    <td>{{$items['default_tax_code']}}</td>
                                    <td>{{$items['balance']}}</td>
                                    <td>{{$items['as_of']}}</td>
                                    <td>
                                        ...
                                        <a href="{{url('accounting/delete/'.$items['id'])}}" onclick="return confirm('Are you sure you want to delete this item?');"><i class="fas fa-trash"></i></a>
                                    </td>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body" style="padding: 0px 0;">
                @if($errors->any())
                    ...
                    @endforeach
                @endif
                <form action="{{url('accounting/add')}}" method="post" id="accounts-forms">
                    @csrf
...
<script>
    ...
    function viewEditProductsAndServices(purpose, id){
        ...
        $.ajax({
            url: "{{url('accounting/get_account_details')}}" + "/" + id,
            method: "GET",
